...
Source: https://developer.collibra.com/workflows/workflow-documentation/Content/Workflows/ExecutionLogic/ref_addterm-delegate.htm
...
The 
AddTerm
 delegate adds the term and returns it with the output variable.
...
Id of the vocabulary that the term will be added in.

typeId

Y

Identify type term for the term - either id or signifier must be set, setting both will cause typeId to be used.
