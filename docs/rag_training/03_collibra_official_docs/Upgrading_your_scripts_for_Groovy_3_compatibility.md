...
Source: https://developer.collibra.com/workflows/workflow-documentation/Content/Workflows/DesignWorkflows/to_groovy3-upgrade.htm
...
Groovy 3 introduces a list of breaking changes that can affect Collibra Groovy workflow scripts. You can consult the full list in the 
Groovy 3 release notes
. There are two breaking changes that can most likely affect Collibra workflows:

If a Groovy switch statement has a default branch, it must be the last branch.

Groovy is now more compliant with the JavaBeans specification for one edge case scenario involving any field having a name starting with an uppercase letter (
GROOVY-9618
).
