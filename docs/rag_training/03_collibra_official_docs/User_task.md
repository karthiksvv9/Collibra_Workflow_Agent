...
Source: https://developer.collibra.com/workflows/workflow-documentation/Content/Workflows/WorkflowDesigner/Process/ref_activities_user_task.htm
...
A link to a form that the user must interact with.
...
Defines a list of users that can become assignee by claiming the task. New tasks default to 
user(${initiator})
.
...
Custom form properties which are used within the legacy or custom form engine.
...
Allows you to invoke Java logic after the creation, reassignment, completion and deletion of the task.

Task ID variable

An optional variable that contains the ID of the task.

Task Completer variable

An optional variable that stores the name of the person who completes the task. The value of the variable is a String representation of the username.
