...
Source: https://developer.collibra.com/workflows/workflow-documentation/Content/Workflows/ExecutionLogic/ref_getattribute-delegate.htm
...
GetAttribute
 and 
RemoveAttribute
 delegate respectively gets or removes attributes identified by the input parameters. Matching can be done by attributeId or by owner and type. If attributeId given the rest of the parameters are not taken into account. Matched/removed attributes are returned as a list with the output variable.
...
Id of the term that the attribute belongs to.

typeId

N

Identify type term of the attribute - either id or signifier must be set, setting both will cause typeId to be used.
