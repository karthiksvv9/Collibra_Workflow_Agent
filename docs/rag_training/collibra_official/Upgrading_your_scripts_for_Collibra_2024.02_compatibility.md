...
Source: https://developer.collibra.com/workflows/workflow-documentation/Content/Workflows/DesignWorkflows/to_spring-boot-3-upgrade.htm
...
We are upgrading the Collibra workflow engine to use Jakarta EE, starting with Collibra version 2024.02. The  Jakarta EE APIs (
jakarta.*
) have a different package structure than Javax EE APIs (
javax.*
), causing workflow scripts that use 
javax.*
 functions to fail after the upgrade.
...
You must restart 
running workflows
 that are impacted by this change if there are still tasks to be executed that use Javax EE 
javax.*
 functions.

Do not deploy updated workflows with Jakarta EE 
jakarta.*
 functions before the 2024.02 release.

Custom workflows

You must update your scripts if you have workflow script tasks or scripts stored in the 
groovy-lib
 directory that use Javax EE 
javax.*
...
The 
javax
 packages that are part of the Java SDK are not impacted by this change and they require no update. Some commonly used 
javax
 Java SDK packages are:
...
When you deploy a new version of a workflow and there is a running instance of the same workflow, the running workflow continues to use the old version until it completes. If there is a Javax EE 
javax.*
